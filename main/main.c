...
"function wladdm(m){"
"fetch('/api/whitelist/add?mac='+m).then(function(r){return r.json()}).then(function(d){L('Added '+m);wlrefresh();scan()})}"
"function wlrm(m){fetch('/api/whitelist/remove?mac='+m).then(function(r){return r.json()}).then(function(d){L('Removed '+m);wlrefresh();scan()})}"
